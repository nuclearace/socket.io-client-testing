...
func connectSocket() {
    socket.connect(timeoutAfter: 0.2) {
        print("failed to connect")
    }
}

socket = SocketIOClient(socketURL: URL(string:"http://localhost:8080/")!, config: [
...
func readInput() {
    ...
    case "joinNsp":
        socket.joinNamespace(nspString)
    default:
        break
    }
...
socket.on("jsonTest") {data, ack in
    print("got json test")
    if var json = data[0] as? NSDictionary {
